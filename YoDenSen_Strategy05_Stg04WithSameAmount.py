...
def backtest(intInvestAmount, intNetProfit, start_date, end_date, hold_days, RSV_Ndays, lstStockSorted, PickStockNumber, data, PathExcelFile, weight='average', benchmark=None, stop_loss=None, stop_profit=None):  
    # portfolio check
    ...
    for sdate, edate in dates:
        ...
        dfSortedStock = YoDenSen_Adv_Sorted(lstStockSorted, 市值, 自由現金流, 股東權益報酬率, 營業利益成長率, 市值營收比, rsv) 
        ...
        # Calculate Return Invest Amount
        floatROERate = float(s.iloc[-1]/s.iloc[0])
        print(floatROERate)
        if floatROERate != 0:            
            intReturnAmount = round((intInvestAmount * floatROERate),0)
        intNetProfit = intReturnAmount-intInvestAmount
        strNetProfit = str(intNetProfit)
        print('Net Profit: '+ strNetProfit)
        input('____')
        
        # Write to excel        
        dfDateROE = pd.DataFrame([strDate, strROE, strNetProfit], ['Date', 'ROE', 'strNetProfit'])
        dfDateROE.columns = ['Date & ROE & NetProfit'] 
        #dfDateROEStock = dfSortedHeadStock.append(dfDateROE)
        dfDateROEStock = dfDateROE.append(dfSortedHeadStock)
        sheet_name = 'RSV'+str(RSV_Ndays)+'_Hold'+str(hold_days)
    ...
    '''    
    return equality, transections, strNetProfit
...
def YoDenSen_Adv_Sorted(lstStockSorted, 市值, 自由現金流, 股東權益報酬率, 營業利益成長率, 市值營收比, rsv):  
    condition1 = (市值 < 1e10)
    condition2 = 自由現金流 > 0
...
if __name__ == '__main__':  
    ...
    for RSV_Ndays in range (50, 101, 50):
        for StockHold_Days in range (15, 16, 15):
            ...
            equality, transections, intReturnProfit = backtest(intInvestAmount, intNetProfit, StartDate, EndDate, StockHold_Days, RSV_Ndays, lstStockSorted, PickStockNumber, Data(), PathExcelFile)
