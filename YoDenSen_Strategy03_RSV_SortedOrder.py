...
def backtest(start_date, end_date, hold_days, RSV_Ndays, lstStockSorted, PickStockNumber, data, PathExcelFile, weight='average', benchmark=None, stop_loss=None, stop_profit=None):  
    # portfolio check
...
def dic_ROE(equality, RSV_Ndays, SortedOrder):
    ...
    dicData = {'RSV_Ndays': RSV_Ndays, 'SortedOrder': SortedOrder, 'SumROE': SumROEPercent_Round}
...
def CheckCondition(intStockNumber):
    strStockNumber = str(intStockNumber)
    市值, 自由現金流, 股東權益報酬率, 營業利益成長率, 市值營收比, rsv = pdData(data, RSV_Ndays)
...
if __name__ == '__main__':  
    ...
    # 股票篩選排序項目
    lstStockSortedItem = ['市值', '自由現金流', 'rsv', '股東權益報酬率', '營業利益成長率', '市值營收比']
    #lstStockSortedItem = ['股東權益報酬率', 'rsv'] 
    ...
    for RSV_Ndays in range (50, 251, 50):
        for SortedOrder in lstStockSortedItem:
            lstStockSorted = [SortedOrder]
            print(SortedOrder)
            print('-'*10 + ' RSV & SortedOrder '  +'-'*10)
            print('RSV_Ndays: ' + str(RSV_Ndays))
            print('SortedOrder: ' + str(SortedOrder))
            ...
            ## Output:
            dicData = dic_ROE(equality, RSV_Ndays, SortedOrder)
