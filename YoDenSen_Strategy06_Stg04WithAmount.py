...
def backtest(intInvestAmount, start_date, end_date, hold_days, RSV_Ndays, lstStockSorted, PickStockNumber, data, PathExcelFile, weight='average', benchmark=None, stop_loss=None, stop_profit=None):  
    # portfolio check
    ...
    for sdate, edate in dates:
        ...
        input('floatROERate')
        if floatROERate != 0:            
            intInvestAmount = round((intInvestAmount * floatROERate),0)
        strReturnAmount = str(intInvestAmount)
        print('Retun Amount: '+strReturnAmount)
        
        # Write to excel        
        dfDateROE = pd.DataFrame([strDate, strROE, strReturnAmount], ['Date', 'ROE', 'ReturnAmount'])
        dfDateROE.columns = ['Date & ROE & ReturnAmount'] 
    ...
    '''    
    return equality, transections, intInvestAmount
...
if __name__ == '__main__':  
    ...
    for RSV_Ndays in range (50, 101, 50):
        for StockHold_Days in range (15, 16, 15):
            ...
            equality, transections, intReturnAmount = backtest(intInvestAmount, StartDate, EndDate, StockHold_Days, RSV_Ndays, lstStockSorted, PickStockNumber, Data(), PathExcelFile)
            ...
            intInvestAmount = intReturnAmount
